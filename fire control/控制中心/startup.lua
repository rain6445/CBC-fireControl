...
======================================================================
    ...
    - [核心] 新增红石输入触发火炮开火功能：
             - UI中选择一个面作为红石输入面。
             - 当该面接收到红石信号时，向所有关联的火炮发送持续的开火信号。
             - 信号消失则停止开火。
    - [UI优化] 调整UI布局，为红石输入选择器腾出空间。
======================================================================]]
...
system.resetProp = function()
    return {
        ...
        maxSelectRange = "480",
        whiteList = {},
        redstoneFireInputSide = "top", -- 新增：红石输入触发开火的面
    }
end
    ...
    if name == "cbc_missile" then
        ...
        return function()
            rednet.send(id, {
                ...
                velocity = t_target.velocity,
                mode = g_group.mode,
                fire = g_group.fire -- 这里会根据 group[ca.group].fire 状态发送开火信号
            }, protocol)
        end
    end
end
...
scanner.run = function()
    while true do
        ...
        for _, g in pairs(group) do
            -- 仅当 autoFire 为 true 时，才由内部计时器控制 fireCd
            if g.autoFire then
                if g.fireCd > 0 then
                    g.fireCd = g.fireCd - 1
                    if g.fireCd == 1 then
            ...
            -- 如果 autoFire 为 false，则 fire 状态由外部（如红石）控制
            -- 此时 fireCd 应该被忽略或重置，以避免干扰
            else
                g.fireCd = 0 -- 确保在非自动开火模式下冷却计时器不影响 fire 状态
            end
        end
...
function absCoordInputWindow:refresh()
    ...
    self.drawW.drawText(46, 96, "--------", 0x666666, 0x000000) -- Changed from ++++++++ to -------- for consistency
...
function absRadarButtons:refreshRadar()
    ...
    local q = {}
    if onVsShip then -- onVsShip is not defined, assuming it means if targeting ships
...
function absRadarButtons:RadarClick(x, y, button)
    if y <= 110 then
        if x < 18 and y < 9 then
            ...
            local q, tgName = {}, ""
            -- onVsShip is not defined, assuming it means if targeting ships
            if group[self.group.index].mode == 3 then 
                if ship then
                    q = quatMultiply(quatList[properties.face], getConjQuat(ship.getQuaternion()))
                    tgName = ship.getName()
                else
...
function absTextField:inputKey(key)
    ...
    if key == 259 or key == 261 then -- backSpace
        if xPos > minXp then
            termUtil.cpX = termUtil.cpX - 1
            if #field > 0 and termUtil.cpX >= minXp then -- 修正索引，确保不删除标签
            ...
            if self.type == "number" then
                ...
                if field == "" or field == "-" then -- 允许空字符串或负号作为临时输入
                    self.key[self.value] = field
                elseif number then
                    self.key[self.value] = number
                else
                    self.key[self.value] = 0 -- 无效数字回退到0
                end
            elseif self.type == "string" then
                self.key[self.value] = field
            end
...
function termUtil:init()
    self.fieldTb = {
        ...
        maxSelectRange = newTextField(properties, "maxSelectRange", 18, 8), -- 调整y坐标
    }
    self.selectBoxTb = {
        redstoneFireInputSide = newSelectBox(properties, "redstoneFireInputSide", 2, 20, 5, "top", "bottom", "left", "right", "front"), -- 新增红石输入选择器
        face = newSelectBox(properties, "face", 2, 12, 7, "south", "west", "north", "east") -- 调整y坐标
    }
...
-- 新增：红石输入触发开火控制函数
local function runRedstoneFireControl()
    local current_fire_state = false -- 跟踪当前的红石输入状态

    while true do
        local input_side = properties.redstoneFireInputSide
        local new_fire_state = redstone.getInput(input_side)

        if new_fire_state ~= current_fire_state then
            current_fire_state = new_fire_state
            
            -- 更新所有 group 的 fire 状态
            for i = 1, #group do
                -- 只有当 autoFire 为 false 时，才允许红石信号控制 fire 状态
                -- 否则 autoFire 模式会覆盖红石信号
                if not group[i].autoFire then
                    group[i].fire = current_fire_state
                end
            end
        end
        sleep(0.05) -- 防止忙循环
...
local run = function()
    parallel.waitForAll(redNet, beats, getGoggles, scanner.run, events, sender, runRedstoneFireControl) -- 将 runRedstoneFireControl 添加到并行任务中
end
